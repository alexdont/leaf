...
(function () {
  ...
  window.LeafHooks.Leaf = {
    ...
    _onVisualKeydown: function (e) {
      ...
      if (e.key === "Enter" && e.shiftKey) {
        // When Shift+Enter bridges a paragraph boundary (cursor at end of
        // <p> with next sibling <p>, or at start of <p> with previous <p>),
        // merge the two paragraphs into a single <p> with an internal <br>.
        // Without this, the browser only inserts a <br> at the cursor and
        // leaves the surrounding <p>s separate — looks merged visually, but
        // round-trips through markdown as paragraph-separated text.
        ...
        keep.appendChild(br);
        var firstMoved = merge.firstChild;
        while (merge.firstChild) {
          keep.appendChild(merge.firstChild);
        }
        ...
        // Tell the input pipeline content changed so the debounced push fires.
